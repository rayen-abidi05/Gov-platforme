...
export default function InspaAssignedPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return inspections
      .filter((i) => i.status === "ASSIGNED" || i.status === "UNDER_REVIEW")
      .filter((i) => {
  ...
  return (
    <>
      <AdminHeader
        ...
        subtitle="Inspections qui vous ont été assignées et en cours de traitement"
      />
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-cream-50/10 bg-olive-950/40 backdrop-blur-md p-5 sm:p-6">
            <InspectionFilters
              ...
              statusOptions={["ASSIGNED", "UNDER_REVIEW"]}
            />
            <div className="mt-5">
              <InspectionsTable
                inspections={filtered}
                emptyLabel="Aucune inspection assignée pour le moment."
              />
            </div>
          </div>
        )}
