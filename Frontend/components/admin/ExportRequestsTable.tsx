...
export default function ExportRequestsTable({
  ...
  // only SENT requests can be picked into a new instance session
  const selectable = requests.filter((r) => r.status === "SENT");
  const allSelectableSelected =
    selectable.length > 0 && selectable.every((r) => selectedIds.includes(r.id));
          ...
          <tr className="border-b border-cream-50/10 text-xs uppercase tracking-wide text-cream-50/50">
            <th className="w-10 px-5 py-3.5">
              <input
                ...
                onChange={() => onToggleSelectAll(selectable.map((r) => r.id))}
