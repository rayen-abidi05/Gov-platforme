...
export default function AgrimMonitoringCard({ agrim }: Props) {
  const percentage = Math.min(
    100,
    Math.round((agrim.consumedKg / agrim.limitKg) * 100)
  );
    ...
    <div className="rounded-xl border border-cream-50/10 bg-olive-950/40 backdrop-blur-md p-5">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-cream-50">
          AGRIM {agrim.reference}
        </p>

        <span className={`text-xs font-medium ${
          isNearLimit ? "text-red-300" : "text-gold-300"
        }`}>
          {percentage}%
        </span>
      </div>
